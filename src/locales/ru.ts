export const ru = {
  // App title
  appTitle: "HostLib Viewer",

  // Menu items
  overview: "Обзор",
  daily: "Суточный",
  hourly: "Часовой",
  edit: "Изменения",
  sys: "Аварии",
  param: "Параметры",
  grs: "ГРС",

  // Archive types
  dailyArchive: "Суточный архив",
  hourlyArchive: "Часовой архив",
  editArchive: "Архив изменений",
  systemArchive: "Системный архив",
  parameters: "Параметры",

  // Table columns
  period: "Период",
  value: "Значение",
  workingVolumePressure: "Раб. объем/перепад",
  workingVolume: "Раб. объем",
  pressure: "Давление",
  outputPressure: "Давление вых.",
  temperature: "Температура",
  density: "Плотность",
  editCounts: "И",
  sysCounts: "А",

  // Tooltips
  changesCount: "Изменения",
  alarmsCount: "Аварии",

  // Actions
  selectLines: "Выберите линии для отображения данных",
  activateDate: "Активируйте фильтр по датам для загрузки данных",
  loading: "Загрузка...",
  noData: "Нет данных для отображения",
  loadingData: "Загрузка данных...",
  cancel: "Отмена",

  // Pagination
  perPage: "На странице",
  page: "Стр.",
  pageDay: "сутки",
  pageWeek: "неделя",
  pageMonth: "месяц",
  pageQuarter: "квартал",
  pageYear: "год",

  // Summary row aggregation
  summaryRow: "Итог",
  aggregateChange: "нажмите, чтобы изменить",
  aggregateSum: "Сумма",
  aggregateAvg: "Среднее",
  aggregateMax: "Максимум",
  aggregateMin: "Минимум",

  // Export
  records: "Записей",
  export: "Экспорт",
  excel: "Excel",
  noDataExport: "Нет данных для экспорта",
  exportError: "Ошибка при экспорте в Excel",

  // Summary
  total: "Итого:",

  // Error messages
  errorLoading: "Ошибка загрузки данных",

  // File names for export
  dailyArchiveFile: "суточный_архив",
  hourlyArchiveFile: "часовой_архив",
  editArchiveFile: "архив_изменений",
  systemArchiveFile: "архив_аварий",
  parametersFile: "параметры",
  enterpriseFile: "предприятия",

  // GRS Report
  grsReport: "Отчет ГРС",
grsTrends: "Тренды ГРС",
  fhpComparison: "Сверка ФХП",
  lineComparison: "Сравнение линий",
  nightConsumption: "Ночные расходы",
  nightConsumptionDescription: "Минимальный расход газа с 00:00 до 05:00 по каждой ГРС",
  nightConsumptionNetDescription: "Минимальный NET объем (без промышленности) с 00:00 до 05:00 по каждой ГРС",
  nightConsumptionChartsTitle: "Графики ночных расходов по линиям",
  nightConsumptionAvgDescription: "Средний NET объем (без промышленности) за 02:00 и 03:00 по каждой ГРС",
  nightReportMin: "Минимальный (00–05)",
  nightReportAvg: "Средний (02–03)",
  nightDayCommercialShort: "Газовые (07–07)",
  nightDayCalendarShort: "Астрономические (00–00)",
  nightDayCommercial: "газовые сутки 07:00–07:00",
  nightDayCalendar: "астрономические сутки 00:00–00:00, вечерние часы с предыдущих суток",
  accidents: "Аварии",
  accidentsReport: "Отчет по авариям",
  accidentsDescription: "Анализ аварийных событий по линиям за выбранный период",
  accidentType: "Тип аварии",
  occurrenceCount: "Количество случаев",
  totalDuration: "Общая длительность",
  totalVolume: "Общий объем",
  accidentDetails: "Детали аварии",
  startTime: "Время начала",
  endTime: "Время окончания",
  duration: "Длительность",
  noAccidentsFound: "Аварий не найдено",
  loadAccidentsData: "Загрузить данные",
  allLines: "Все линии",
  linesShown: "Линии",
  table: "Таблица",
  chart: "График",
  phaseArchives: "Загрузка архивных данных…",
  phaseEnterprise: "Опрос предприятий",
  phaseAggregating: "Обработка данных…",
  phaseWaiting: "Ожидание другого опроса…",
  phaseCalculating: "Расчёт…",
  phaseDevicesDone: "приборов",
  searchLine: "Поиск линии...",
  selectAll: "Все",
  selectNone: "Ни одной",
  onlyThis: "только",
  searchEventType: "Поиск типа события",
  nothingFound: "Ничего не найдено",
  totalAccidents: "Всего аварий",
  accidentTypes: "Типов аварий",

  // Date/Time
  from: "С",
  to: "До",
  date: "Дата",
  time: "Время",

  // Additional translations for DataTable
  editType: "Тип вмешательства",
  oldValue: "Старое значение",
  newValue: "Новое значение",
  operationType: "Тип аварии",

  // Chart labels
  chartTitle: "График данных",
  noChartData: "Нет данных для отображения графика",
  updatingChart: "Обновление графика...",
  renderingChart: "Отрисовка графика...",
  chartPreparation: "График готовится к отображению...",

  // Chart series labels
  volumeLabel: "Объем",
  workingVolumeDpLabel: "Раб. объем/перепад",
  pressureLabel: "Давление",
  temperatureLabel: "Температура",
  densityLabel: "Плотность",
  co2Label: "CO2 (%)",
  n2Label: "N2 (%)",
  maxPressureLabel: "Макс. давление",
  minPressureLabel: "Мин. давление",
  maxTemperatureLabel: "Макс. температура",
  minTemperatureLabel: "Мин. температура",
  oldValueLabel: "Старое значение",
  newValueLabel: "Новое значение",

  // TreeView
  nodeListTitle: "Список узлов учета",
  selectedLine: "Выбрана линия:",
  treeSearch: "Поиск линии...",
  calcObject: "Объект",
  calcAddress: "Адрес",
  calcLine: "Линия",
  calculator: "Вычислитель",
  loadingError: "Ошибка загрузки данных",
  noDataToDisplay: "Нет данных для отображения",

  // Date pickers
  periodStart: "Начало периода",
  periodEnd: "Конец периода",
  selectDateTime: "Выберите дату и время",
  today: "Сегодня",
  apply: "Применить",
  setToday: "Установить текущую дату",

  // GRS Report errors and messages
  calculationError: "Ошибка при расчете отчета",
  unknownReportError: "Неизвестная ошибка при получении отчета",
  serverConnectionError: "Ошибка подключения к серверу",
  grsVolumeTitle: "Объем по ГРС за последние 24 часа",
  grsTotalVolume: "ГРС всего",
  unknownDataFormat: "Неизвестный формат данных",
  volumeUnit: "м³",
  pressureUnit: "кг/см²",
  pressureUnits: "Единицы давления",
  volume: "Объем",
  pressureIn: "Pвх",
  pressureOut: "Pвых",
  noGrsLinesConfigured: "Не настроены линии ГРС",
  noDataAvailable: "Нет доступных данных",
  errorLoadingData: "Ошибка загрузки данных",
  groupOnlyDailyHourly: "Узел дерева показывает объёмы только в суточном и часовом архивах",
  groupHasNoLines: "В этом узле нет линий",
  linesReported: "Отчитались",
  noDataForPeriod: "Нет данных за выбранный период",
  refresh: "Обновить",
  close: "Закрыть",
  exportToExcel: "Экспорт в Excel",
  errorExportingData: "Ошибка при экспорте данных",
  error: "Ошибка",
  selectPeriod: "Выберите период",
  calculatingTrends: "Расчет трендов...",
  grsConsumptionTrends: "Тренды потребления ГРС",
  grsTracksDescription: "График показывает процентное распределение потребления газа по каждой ГРС относительно общего объема за период",

  // Enterprise Overlay
  enterpriseOverlay: "Промышленность",
  enterpriseNetVolumeInfo: "Показывается Net Volume (разница между объемом линии и промышленностью) и Total Enterprise",
  loadingEnterpriseData: "Загрузка данных предприятий...",
  netVolume: "Net Volume (Линия - Предприятия)",
  totalEnterpriseVolume: "Суммарный объем предприятий",
  enterpriseNoData: "Нет данных по предприятиям за выбранный период",

  // Virtual Lines
  virtualLines: "Виртуальные линии",
  virtualLineTooltip: "Виртуальная линия (агрегация нескольких физических линий)",
  dpdLineTooltip: "Линия ДПД (данные из API ДПД по серийному номеру корректора)",
  notAvailableForVirtualLines: "Недоступно для виртуальных линий",
  virtualLinesSupportOnlyDailyHourly: "Виртуальные линии поддерживают только суточный и часовой архивы",

  // Overview Tab
  grsOverviewTitle: "Обзор ГРС",
  lastUpdate: "Данные актуальны на",
  total24hVolume: "Общий объем за 24 часа",
  comparedToPrevious: "по сравнению с предыдущими 24ч",
  linePressures: "Давление по линиям",
  linesCount: "линий",
  collapseAll: "Свернуть все",
  expandAll: "Развернуть все",
  noLinesInLumg: "Нет линий для отчёта",
  hourlyFlowRates: "Часовые расходы",
  lastHour: "Последний час",
  previousHour: "Предыдущий час",
  change: "Изменение",
  volumeComparison24h: "Сравнение объемов за 24ч",
  current24h: "Текущие 24ч",
  previous24h: "Предыдущие 24ч",
  refreshNow: "Обновить",
  autoRefresh: "Авто-обновление",
  activeLines: "Активные линии",
  allLumgs: "Все ЛУМГ",
  nextRefreshIn: "Следующее обновление через",
  lineName: "Линия",
  flowRate: "Расход",

  // Short versions for compact tables
  lastHourShort: "Посл. час",
  previousHourShort: "Пред. час",
  current24hShort: "Тек. 24ч",
  previous24hShort: "Пред. 24ч",
  changeShort: "Δ",

  // Differential Pressure (dP)
  differentialPressure: "Перепад давления",
  differentialPressureShort: "dP",
  dpRange: "Диапазон dP",
  currentDp: "Текущий dP",
  flowInWorkingConditions: "Расход в р.у.",
  currentValue: "Текущее",
  maxValue24h: "Макс. 24ч",
  staleData: "Данные устарели",
  staleBehindBy: "отстают на",
  unitHourShort: "ч",
  unitDayShort: "дн",
  unitMinShort: "мин",
  unitSecShort: "с",

  // Enterprise accidents report
  entAccPoll: "Выберите период и нажмите «Опросить»",
  entAccNone: "Аварий за этот период не найдено",
  entAccType: "Тип аварии",
  entAccFirst: "Первое появление",
  entAccLast: "Последнее появление",
  entAccDuration: "Суммарная длительность",
  entAccCount: "Появлений",
  entAccObjects: "Объектов",
  entAccChecked: "Проверено наших приборов",
  entAccOfOurs: "наших в филиале",
  entAccWithEvents: "Приборов с авариями",
  entAccTypes: "Типов аварий",
  entAccUntranslated: "Нет в справочнике",
  entAccPhaseList: "Получение перечня приборов",
  entAccPhasePolling: "Опрос приборов",
  entAccSearchType: "Поиск по типу...",

  // Enterprise Poll Analysis
  enterprisePoll: "Опрос предприятий",
  unpolledEnterprises: "Нет опроса",
  searchEnterprise: "Поиск предприятия...",
  correctorAny: "Корректор: все",
  correctorModel: "Модель корректора",
  correctorShort: "Корректор",
  gsmModem: "GSM-модем",
  withModem: "с модемом",
  withoutModem: "без модема",
  pollResults: "Результаты опроса",
  dailyPoll: "Суточный",
  hourlyPoll: "Часовой",
  poll: "Опросить",
  selectEnterprise: "Выберите предприятие",
  noPollData: "Нет данных опроса",
  enterpriseList: "Список предприятий",
  pollDate: "Дата опроса",
  totalEnterprises: "Всего предприятий",
  activeEnterprises: "Активных",
  unpolledCount: "Не опрошенных",
  unpolledChecked: "Проверено",
  enterprise: "Предприятие",
  unpolledByName: "Поименно",
  unpolledSummary: "Сводная",
  unpolledTotal: "Не опрошено",
  unpolledOutOf: "из",
  unpolledOn: "Включённые",
  unpolledOff: "Отключённые",
  unpolledByCorrector: "По типам корректоров",
  unknownCorrector: "Тип не указан",
  resetFilters: "Сбросить фильтры",
  unpolledNone: "Все предприятия отвечают",
  unpolledStart: "Нажмите «Обновить», чтобы проверить",
  polledAt: "Опрошено",
  reportStale: "Отчёт старше часа — обновите, чтобы увидеть текущее состояние",
  unpolledRowHint: "Выбрать это предприятие для опроса",
  back: "Назад",
  stop: "Остановить",
  lastPollDate: "Последний опрос",
  neverPolled: "Не опрашивалось",
  pollError: "Ошибка опроса",
  loadingEnterprises: "Загрузка предприятий...",
  noEnterpriseSelected: "Выберите предприятие для просмотра данных",
  enterpriseVolumeChart: "График объемов",
  showVolume: "Объем",
  showTemperature: "Температура",
  showPressure: "Давление",
  withoutLine: "Без линии",
  correctorType: "Тип корректора",
  searchCorrector: "Поиск модели...",
  clearSelection: "Очистить",
  correctorNumber: "Номер корректора",
  channelNumber: "Номер канала",
  exportExcel: "Экспорт в Excel",
  status: "Статус",
  statusEnabled: "Включена",
  statusDisabled: "Отключена",
  inactive: "Неактивное",
  inactiveNotInReports: "Опрашивается вручную, но в отчёты не входит",

  // Shared UI
  branch: "Филиал",
  unknownLine: "Линия",
  flowUnit: "м³/ч",
  dpUnit: "кг/м²",
  noPressureData: "Нет данных о давлении",
  selectBranchPrompt: "Выберите филиал для отображения данных",

  // Error messages
  noLumgsForBranch: "Нет ЛУМГов для выбранного филиала",
  noReportLinesForBranch: "Нет линий для отчета в выбранном филиале",
  noDataFor24h: "Нет данных за последние 24 часа",
  noLinesData: "Не удалось получить данные линий",
  noTimeBoundsError: "Не удалось определить временные границы данных",


  // Enterprise control
  enterpriseSettings: "Настройки промышленности",
  enableEnterprise: "Включите промышленность",

  // Param archive columns
  paramMaxDp: "Макс. ΔP",
  paramMinDp: "Мин. ΔP",
  // Meter-based lines: the same two fields hold flow limits, not a dP band.
  paramMaxFlow: "Макс. расход",
  paramMinFlow: "Мин. расход",
  paramRadius: "Радиус",
  paramSuYear: "SU год",
  paramMaxP: "Макс. P",
  paramMinP: "Мин. P",
  paramMaxT: "Макс. T",
  paramMinT: "Мин. T",

  // Flow rate calculation (FlowRateCalc)
  fcMenuTitle: "Расчёт расхода",
  fcTitle: "Расчёт объёма газа",
  fcSelectConverter: "Выберите преобразователь расхода:",
  fcOrificeDevice: "сужающее устройство",
  fcMeter: "счётчик",
  fcGeneralParams: "Общие параметры",
  fcOrificeParams: "Параметры для сужающего устройства",
  fcMeterParams: "Параметры для счётчика",
  fcResults: "Результаты",
  fcCalculate: "Расчёт",
  fcReset: "Сбросить",
  copy: "Копировать",
  copied: "Скопировано",
  copyError: "Не удалось скопировать в буфер обмена",
  // What's new dialog
  whatsNewTitle: "Что нового",
  whatsNewNewer: "Новее",
  whatsNewOlder: "Старее",
  // Auto-fill from line
  fcLineFillTitle: "Заполнить из линии",
  fcLineFillBranch: "Филиал",
  fcLineFillCalc: "Вычислитель",
  fcLineFillLine: "Линия",
  fcLineFillAllBranches: "Все филиалы",
  fcLineFillAllCalcs: "Все вычислители",
  fcLineFillSelectLine: "— выберите линию —",
  fcLineFillLoading: "Загрузка данных линии…",
  fcLineFillPulledFrom: "Данные подтянуты с линии",
  fcLineFillAt: "за",
  fcLineFillNoParams: "Нет данных параметров для этой линии",
  fcLineFillNoHourly: "Нет часовых данных для этой линии",
  fcLineFillHint: "Подтянутые значения можно отредактировать перед расчётом",
  fcAsOfLatest: "Актуальные",
  fcAsOfMoment: "На момент",
  fcAsOfLatestHint: "Последние доступные параметры и последняя часовая запись",
  fcAsOfMomentHint: "Параметры, действовавшие на этот момент, и запись за этот час",
  fcAsOfNoRecord: "Нет часовой записи за",
  // Input labels
  fcDensityLabel: "Плотность:",
  fcCo2Label: "Диоксид углерода:",
  fcN2Label: "Азот:",
  fcPressureTypeLabel: "Тип давления:",
  fcAtmPressureLabel: "Атм. давление:",
  fcPressureLabel: "Давление:",
  fcTemperatureLabel: "Температура:",
  fcOtborLabel: "Тип отбора:",
  fcDpLabel: "Перепад давления:",
  fcMatPipeLabel: "Материал ИТ:",
  fcRoughnessLabel: "Шероховатость, мм:",
  fcMatOrificeLabel: "Материал СУ:",
  fcEdgeRadiusLabel: "Радиус кромки, мм:",
  fcTimeTypeLabel: "Тип времени СУ:",
  fcTimeOrificeLabel: "Время СУ, год:",
  fcWorkVolumeLabel: "Объём в рабочих условиях:",
  // Dropdown values
  fcKstPlaceholder: "Сделайте выбор ...",
  fcPAbs: "абсолютное",
  fcPGauge: "избыточное",
  fcOtborCorner: "угловой",
  fcOtborRad: "трёхрадиусный",
  fcOtborFlange: "фланцевый",
  fcTimeOp: "эксплуатации",
  fcTimeInterctrl: "межконтрольный",
  // Units
  fcUnitYear: "год",
  fcUnitM3h: "м³/час",
  fcUnitM3hStd: "м³/час в с.у.",
  fcUnitM3Std: "м³ в с.у.",
  // Result group titles
  fcGasParams: "Параметры газа",
  fcPseudocritical: "Псевдокритические параметры",
  fcOrificeGroup: "Сужающее устройство",
  fcVolumeReduction: "Приведение объёма",
  // Result rows
  fcGamma: "γ — относительная плотность",
  fcZ: "Z — коэффициент сжимаемости",
  fcRhoW: "ρ<sub>р</sub> — плотность рабочая",
  fcMu: "μ — динамическая вязкость",
  fcC: "C — коэффициент расхода",
  fcStdOrifice: "Расход через СУ",
  fcZc: "Z<sub>с</sub> — фактор сжимаемости при ст. условиях",
  fcKcompr: "K = Z/Z<sub>с</sub> — коэффициент сжимаемости",
  fcPRangeAbs: "Абсолютное давление должно быть 0,1…11 МПа",
  fcKsh: "K<sub>ш</sub> — коэффициент шероховатости",
  fcKbl: "K<sub>п</sub> — коэффициент притупления кромки",
  fcEps: "ε — коэффициент расширения",
  fcKp: "K<sub>p</sub> — коэффициент приведения",
  fcQw: "Q<sub>р</sub> — расход (раб. условия)",
  fcQstOrifice: "Q<sub>ст</sub> — расход (ст. условия)",
  fcVstMeter: "V<sub>ст</sub> — объём (ст. условия)",
  // Placeholder
  fcEnterParamsPrompt: "Введите параметры и нажмите",
  // Validation
  fcEnterValue: "Введите значение",
  fcMin: "Мин",
  fcMax: "Макс",
  fcSelectKst: "Выберите метод Кст",
  fcFixErrors: "Проверьте выделенные поля",
  fcRangeCo2N2: "Диапазон: 0–16 %",
  fcPMustPositive: "P должно быть > 0",
  fcDpMustLessP: "ΔP должно быть < P",
  fcBetaRange: "— должно быть 0.10–0.75",

  // Auth / shell (were hardcoded in the old app)
  loginTitle: "HLViewer",
  loginSubtitle: "Мониторинг ГРС",
  loginUsername: "Логин",
  loginPassword: "Пароль",
  loginRemember: "Запомнить меня",
  loginSubmit: "Войти",
  loginSubmitting: "Вход...",
  loginError: "Ошибка входа",
  sessionRightsChanged: "Ваши права доступа изменил администратор. Войдите в систему заново.",
  sessionDomainLoginOff: "Вход по доменным учётным данным отключён на сервере. Обратитесь к администратору.",
  appLoading: "Загрузка...",
  logout: "Выйти",
  languageLabel: "Язык",
  themeLight: "Светлая тема",
  themeDark: "Тёмная тема",
  roleAdmin: "Администратор",
  roleViewer: "Наблюдатель",
  navReports: "Отчёты",
  navPoll: "Опрос",
  navAdmin: "Администрирование",
  navFlowCalc: "Расчёт расхода"
};
